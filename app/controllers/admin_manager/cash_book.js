...
class Admin_cash_book extends Controller{
	...
	static async get_data(req, res){
        try{
			...
			let match = {
				$and: [ {company :req.session.user.company._id, isActive:true, createdAt: {$gte: new Date(start_time[2],start_time[1]-1,start_time[0],0,0,0), $lt: new Date(start_time[2],start_time[1]-1,start_time[0]+1,0,0,0)}}] 
			}
			let match_current_fund = {
				$and: [ {company :mongoose.Types.ObjectId(req.session.user.company._id), isActive:true, createdAt: {$gte: new Date(start_time[2],start_time[1]-1,start_time[0],0,0,0), $lt: new Date(start_time[2],start_time[1]-1,start_time[0]+1,0,0,0)}}] 
			}
			let match_begin_fund = {
				$and: [{company: mongoose.Types.ObjectId(req.session.user.company._id), isActive:true, createdAt: {$gte: new Date(start_time[2],0,1,0,0,0), $lt: new Date(start_time[2],start_time[1]-1,end_time[0]-1,23,59,59)}}]
			}
